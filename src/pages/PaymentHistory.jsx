...
export default function PaymentHistory() {
  ...
  const toggleSelectAll = (checked) => {
    setSelected(checked ? data.map((row) => row.id) : []);
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-6">
        <div className="flex flex-col md:flex-row md:items-center md:space-x-3 gap-3 w-full">
          ...
          <div className="flex space-x-2">
            <Button variant="outline" size="icon" className="bg-gray-50 rounded-lg hover:bg-gray-100">
        ...
        <TableBody>
          {data.map((row) => (
            <TableRow key={row.id}>
              ...
              <TableCell className="text-[#28643B] cursor-pointer">View All</TableCell>
              <TableCell>
                <span className={`font-medium ${row.status ? "text-[#15C26B]" : "text-[#EC4F3C]"}`}>
                  {row.status ? "Paid" : "Decline"}
                </span>
              </TableCell>
